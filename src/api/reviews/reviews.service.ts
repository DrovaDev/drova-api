...
export class ReviewsService {
  ...
  async submitGuestBusinessReview(dto: SubmitGuestReviewDTO): Promise<IResponse> {
  ...
  async getBusinessReviews(businessId: string, query: ReviewQueryDTO): Promise<IResponse> {
    return this.getReviewsByTarget(businessId, ReviewTargetType.BUSINESS, query);
  }

  async getRiderReviews(riderId: string, query: ReviewQueryDTO): Promise<IResponse> {
  ...
  private async getReviewsByTarget(
    ...
    const { count } = await this.reviewsDb.findReviewsByTarget(targetId, targetType, {
      offset: 0,
      limit: 0,
      sortOrder,
    });
    ...
    const { reviews } = await this.reviewsDb.findReviewsByTarget(targetId, targetType, {
      offset,
      limit,
      sortOrder,
    });

    const averageRating = await this.reviewsDb.getAverageRating(targetId, targetType);
  ...
  private async resolveOrderByReference(referenceCode: string): Promise<Orders> {
  ...
  private assertRiderReviewEligibility(order: Orders, targetType: ReviewTargetType): void {
  ...
  private async assertNoDuplicateReview(
    ...
    const existing = await this.reviewsDb.findExistingReview(orderId, targetType);
    if (existing) {
  ...
  private resolveTargetId(order: Orders, targetType: ReviewTargetType): string {
    return targetType === ReviewTargetType.RIDER ? order.riderId! : order.businessId;
  }
